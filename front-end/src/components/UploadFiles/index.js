...
const UploadFiles = ({id}) => {
    ...
    function SaveSingleFile() {
            if(fileInput.current.files.length !== 0){   
                ...
                fetch('/institution/saveSingleFile', {
                    method: 'POST',
                    body: formData,
                    credentials: 'same-origin',
                    headers: {
    ...
    return (
        <InstitutionsContainerNoCenter>
            ...
            <BoxContainer style={{alignSelf: "center"}}>
                <br/>
                <h2 style={{color: "#4d96b8"}}>Add file in subject</h2>
                <br/>
                <h4>{fileName}</h4>
                {singleFile === false && <ButtonBlue to="#" onClick={HandleSingleFileClick}>Select File</ButtonBlue>}
                {singleFile === true && <ButtonBlue to="#" onClick={SaveSingleFile}>Upload File</ButtonBlue>}
